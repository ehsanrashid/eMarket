...
#ifndef _SBE_CHAR32STR_CXX_H_
#define _SBE_CHAR32STR_CXX_H_
...
class Char32str {
   ...
   public:
    ...
    Char32str(char *buffer, const std::uint64_t offset,
        ...
        if (SBE_BOUNDS_CHECK_EXPECT(((m_offset + 32) > m_bufferLength),
                                    false)) {
            throw std::runtime_error("buffer too short for flyweight [E107]");
        }
    }

    Char32str(char *buffer, const std::uint64_t bufferLength,
              const std::uint64_t actingVersion)
        : Char32str(buffer, 0, bufferLength, actingVersion) {}

    Char32str(char *buffer, const std::uint64_t bufferLength)
        : Char32str(buffer, 0, bufferLength, sbeSchemaVersion()) {}

    Char32str &wrap(char *buffer, const std::uint64_t offset,
                    const std::uint64_t actingVersion,
                    const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
    ...
    static SBE_CONSTEXPR std::uint64_t vLength() SBE_NOEXCEPT { return 32; }
    ...
    Char32str &v(const std::uint64_t index, const char value) {
        if (index >= 32) {
    ...
    Char32str &putV(std::span<const char, N> src) SBE_NOEXCEPT {
        static_assert(N <= 32, "array too large for putV");
...
#ifdef SBE_USE_SPAN
    template <typename T>
    Char32str &putV(T &&src) SBE_NOEXCEPT
        requires(std::is_pointer_v<std::remove_reference_t<T>> &&
                 !std::is_array_v<std::remove_reference_t<T>>)
#else
    Char32str &putV(const char *const src) SBE_NOEXCEPT
#endif
    {
...
#ifdef SBE_USE_SPAN
    template <std::size_t N>
    Char32str &putV(const char (&src)[N]) SBE_NOEXCEPT {
...
#ifdef SBE_USE_STRING_VIEW
    Char32str &putV(const std::string_view str) {
...
#else
    Char32str &putV(const std::string &str) {
        const std::size_t srcLength = str.length();
        if (srcLength > 32) {
            throw std::runtime_error("string too large for putV [E106]");
        }
    ...
    friend std::basic_ostream<CharT, Traits> &operator<<(
        std::basic_ostream<CharT, Traits> &builder, Char32str &writer) {
        builder << '{';
        builder << R"("v": )";
