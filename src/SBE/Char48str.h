...
#ifndef _SBE_CHAR48STR_CXX_H_
#define _SBE_CHAR48STR_CXX_H_
...
class Char48str {
   ...
   public:
    ...
    Char48str(char *buffer, const std::uint64_t offset,
        ...
        if (SBE_BOUNDS_CHECK_EXPECT(((m_offset + 48) > m_bufferLength),
                                    false)) {
            throw std::runtime_error("buffer too short for flyweight [E107]");
        }
    }

    Char48str(char *buffer, const std::uint64_t bufferLength,
              const std::uint64_t actingVersion)
        : Char48str(buffer, 0, bufferLength, actingVersion) {}

    Char48str(char *buffer, const std::uint64_t bufferLength)
        : Char48str(buffer, 0, bufferLength, sbeSchemaVersion()) {}

    Char48str &wrap(char *buffer, const std::uint64_t offset,
                    const std::uint64_t actingVersion,
                    const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
    ...
    static SBE_CONSTEXPR std::uint64_t vLength() SBE_NOEXCEPT { return 48; }
    ...
    Char48str &v(const std::uint64_t index, const char value) {
        if (index >= 48) {
    ...
    Char48str &putV(std::span<const char, N> src) SBE_NOEXCEPT {
        static_assert(N <= 48, "array too large for putV");
...
#ifdef SBE_USE_SPAN
    template <typename T>
    Char48str &putV(T &&src) SBE_NOEXCEPT
        requires(std::is_pointer_v<std::remove_reference_t<T>> &&
                 !std::is_array_v<std::remove_reference_t<T>>)
#else
    Char48str &putV(const char *const src) SBE_NOEXCEPT
#endif
    {
...
#ifdef SBE_USE_SPAN
    template <std::size_t N>
    Char48str &putV(const char (&src)[N]) SBE_NOEXCEPT {
...
#ifdef SBE_USE_STRING_VIEW
    Char48str &putV(const std::string_view str) {
...
#else
    Char48str &putV(const std::string &str) {
        const std::size_t srcLength = str.length();
        if (srcLength > 48) {
            throw std::runtime_error("string too large for putV [E106]");
        }
    ...
    friend std::basic_ostream<CharT, Traits> &operator<<(
        std::basic_ostream<CharT, Traits> &builder, Char48str &writer) {
        builder << '{';
        builder << R"("v": )";
